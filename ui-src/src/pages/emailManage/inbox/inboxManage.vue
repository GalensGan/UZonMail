    ...
    <q-table class="col full-height" :rows="rows" :columns="columns" row-key="id" v-model:pagination="pagination" dense
      ...
      <template v-slot:top-left>
        <div class="row justify-start q-gutter-sm">
          ...
          <ExportBtn tooltip="导出收件箱模板" @click="onExportInboxTemplateClick" />
...
async function getRowsNumberCount (filterObj: TTableFilterObject) {
  const { data } = await getBoxesCount(emailGroupRef.value.id, 2, filterObj.filter)
  return data
}
async function onRequest (filterObj: TTableFilterObject, pagination: IQtableRequestParams) {
  const { data } = await getBoxesData<IOutbox>(emailGroupRef.value.id, 2, filterObj.filter, pagination)
  return data
}
