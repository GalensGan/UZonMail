...
namespace SendMultipleEmails.Pages
{
    public class Send_NewViewModel : ScreenChild
    {
        ...
        {
            base.OnViewLoaded();
            Send_NewView view = (Send_NewView)this.View;
        ...
        {
            // 读取模板内容
            string content = Store.TemplateManager.GetTemplate(Store.TemplateManager.GetTemplateFiles()[0].FullName);
            ...
            // 对比提供的个人数据中是否存在
            List<string> tableNames = null;  // Store.PersonalDataManager.GetTableNames(Store.PersonalDataManager.PersonalData.variablesTable);
            ...
            foreach (DataRow receiverRow in receivers)
            {
                ...
                // 按姓名对应，找到发件人
                DataRow[] receiverDataRows = null; //Store.PersonalDataManager.PersonalData.variablesTable.Select(string.Format("{0}='{1}'", keyColumn, receiver.UserId));
