...
{
    class AddReceiversViewModel : ScreenChild
    {
        private static readonly ILog _logger = LogManager.GetLogger(typeof(AddReceiversViewModel));
        public AddReceiversViewModel(Store store) : base(store)
        {
            ...
            try
            {
                using (Stream fs = new FileStream(ExcelFullPath, FileMode.Open, FileAccess.Read, FileShare.Read))
                {
                    ...
                    for (int i = 0; i < totalCount; i++)
                    {
                        ...
                        Store.PersonalDataManager.AddReceiver(person, false);
                        successNum++;
