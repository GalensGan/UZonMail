...
if (-not (Get-Command yarn -ErrorAction SilentlyContinue)) {
    Write-Host "请先安装 yarn 环境, 并添加到 path 环境中" -ForegroundColor Red
    return
}
...
if (-not (Get-Command node -ErrorAction SilentlyContinue)) {
    Write-Host "请先安装 node 环境, 并添加到 path 环境中" -ForegroundColor Red
    return
}
...
if (-not (Get-Command dotnet -ErrorAction SilentlyContinue)) {
    Write-Host "请先安装 dotnet 环境, 并添加到 path 环境中" -ForegroundColor Red
    return
}
...
if (-not (Get-Command docker -ErrorAction SilentlyContinue)) {
    Write-Host "请先安装 docker, 并添加到 path 环境中" -ForegroundColor Red
    return
}
...
$serviceDist = "$scriptRoot/build/service-docker"
dotnet publish -c Release -o $serviceDist -r linux-x64 --self-contained false
# 创建 public 目录
...
$versionParts = $serviceVersion.Split('.')
$filteredVersionParts = $versionParts[0..($versionParts.Length - 1)] | Where-Object { $_ -ne "0" }
$cleanVersion = $filteredVersionParts -join '.'

# 回到根目录
Set-Location -Path $scriptRoot

# 调用 docker 命令进行构建，指定Dockerfile位置和构建上下文
Write-Host "开始构建 Docker 镜像..." -ForegroundColor Yellow
$dockerfilePath = Join-Path -Path $scriptRoot -ChildPath "Dockerfile"
docker build -f $dockerfilePath -t uzon-mail:$cleanVersion $serviceDist
